-- =================================================================
-- Octopus Messenger 合并数据库迁移
-- 版本: 1.0.0 (所有功能合并)
-- 创建日期: 2024-01-01
-- 包含: 基础架构 + Discord + Intercom + 租户模式 + 用户标签 + 群组权限 + 商户系统
-- =================================================================
...
CREATE TABLE IF NOT EXISTS intercom_contacts (
    ...
    last_replied_at BIGINT,
    last_email_opened_at BIGINT,
    last_email_clicked_at BIGINT,
...
CREATE TABLE IF NOT EXISTS tenant_training_data (
    ...
    -- 数据分类
    data_type VARCHAR(50) NOT NULL,          -- conversation, classification, response, intent等
    category VARCHAR(100),                   -- 数据分类
    
    -- 原始数据
    input_data JSONB NOT NULL,               -- 输入数据
    expected_output JSONB,                   -- 预期输出
    actual_output JSONB,                     -- 实际输出
    
    -- 质量评估
    quality_score DECIMAL(3,2),              -- 质量评分 0-1
    reviewed BOOLEAN DEFAULT false,          -- 是否已审核
    approved BOOLEAN DEFAULT false,          -- 是否已批准
...
DROP TRIGGER IF EXISTS update_message_classifications_updated_at ON message_classifications;
...
DROP TRIGGER IF EXISTS update_tenant_training_data_updated_at ON tenant_training_data;
CREATE TRIGGER update_tenant_training_data_updated_at 
    BEFORE UPDATE ON tenant_training_data 
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE INDEX IF NOT EXISTS idx_group_permission_rules_priority ON group_permission_rules(priority);
...
CREATE INDEX IF NOT EXISTS idx_intercom_contacts_tenant ON intercom_contacts(tenant_id);
CREATE INDEX IF NOT EXISTS idx_intercom_contacts_bot_config ON intercom_contacts(bot_config_id);
CREATE INDEX IF NOT EXISTS idx_intercom_contacts_id ON intercom_contacts(contact_id);
CREATE INDEX IF NOT EXISTS idx_intercom_contacts_email ON intercom_contacts(email);
...
CREATE INDEX IF NOT EXISTS idx_discord_interactions_type ON discord_interactions(interaction_type);

-- 租户训练数据索引
CREATE INDEX IF NOT EXISTS idx_tenant_training_data_tenant ON tenant_training_data(tenant_id);
CREATE INDEX IF NOT EXISTS idx_tenant_training_data_type ON tenant_training_data(data_type);
CREATE INDEX IF NOT EXISTS idx_tenant_training_data_category ON tenant_training_data(category);
CREATE INDEX IF NOT EXISTS idx_tenant_training_data_reviewed ON tenant_training_data(reviewed);
CREATE INDEX IF NOT EXISTS idx_tenant_training_data_approved ON tenant_training_data(approved);
...
CREATE INDEX IF NOT EXISTS idx_auto_reply_rules_platform ON auto_reply_rules(platform);
...
-- 组合索引（优化查询性能）
CREATE INDEX IF NOT EXISTS idx_messages_tenant_platform_created ON messages(tenant_id, platform, created_at DESC);
...
COMMENT ON TABLE discord_interactions IS 'Discord交互记录表';
COMMENT ON TABLE tenant_training_data IS '租户训练数据表';
...
SELECT 'Octopus Messenger 合并迁移完成！所有功能已整合到一个文件中。' as message; 